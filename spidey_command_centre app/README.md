...
## Merged team submission

Everyone on the team built a different piece of this system against the
same brief. This version merges the working parts of each into one app,
built on the Command Centre codebase (menu-driven CLI, service layer,
status transitions):

* **Road-network routing** (from the mission-planner submission) —
  `locations.py` no longer invents straight-line distances from a hashed
  coordinate. It now uses a real graph of neighbourhood locations and
  roads (`CITY_MAP`) with Dijkstra's algorithm, so "Next Mission" shows
  a genuine shortest route, including multi-stop ones (e.g. Queens
  Street → Queens Residence → Park Avenue → Central Mall → Metro
  Station), not just a straight HQ → destination hop. The trade-off:
  a location now has to exist on the map to be routable, so free-typed
  ("Other") locations are matched against it case-insensitively instead
  of being accepted as-is.
* **Duplicate-incident detection** (from the incident-intake submission)
  — `incident_service.find_duplicate()` flags an existing *active*
  incident with the same type and location, and the report flow warns
  and asks the operator to confirm before logging it as a separate
  incident, instead of silently double-logging the same fire.
* **Incident type list merged** — both teams' incident-type options are
  combined (deduplicated) into one menu.
* **Update Incident selects by ID** (per the spec) — the operator types
  the incident ID rather than picking from a list, so an invalid ID is
  a real, handled input path (shown, tested, and never crashes) instead
  of something a menu could never produce.
* **Not merged, on purpose:** the `card.py` submission's simpler scoring
  formula (`severity + people*2 + location_category`) and its plain
  `input()`-based menu. The Command Centre's `priority_service.py`
  formula is the one documented against the spec's worked example
  (HIGH + 17 people → 47) and already covered by tests, so it stays the
  single source of truth for priority scoring rather than having two
  competing formulas in the same app.
...
```
31 tests, all passing — covering every service function, every status
transition (valid and invalid), invalid/unknown incident IDs, empty-state
edge cases (no incidents, no active incidents, no available mission),
duplicate detection, road-network routing (including an unreachable
location), and both worked examples from the spec (priority 47 for a
HIGH incident with 17 people; mission score 41 at 6 km distance).
...
* **Severity** and **description** — menu of presets ("Custom" for
  description).
* **Which incident to update** — the spec calls for selecting by ID, so
  this one is typed rather than picked from a list: the active incidents
  are shown for reference, the operator types an ID, and an unknown ID
  is rejected with a clear message instead of crashing — a genuine,
  testable "invalid incident ID" path.
* **What status to move an incident to** — once a valid incident is
  found, the menu only ever shows the status(es) that incident is
  actually allowed to move to next (e.g. a `REPORTED` incident only
  offers `IN_PROGRESS`; a `RESOLVED` incident offers nothing, and the
  operator is told why). An invalid transition literally can't be
  selected — `incident_service.update_status` still re-validates it as
  a safety net in case the two ever drift apart.
