...
what status to move an incident to, whether to confirm or exit) — the
operator picks a number from a list rather than typing free text.
Free text remains where a menu genuinely can't cover it: an "Other"
type/location, a custom description, an exact headcount, and — per the
spec — the incident ID when updating an incident, which is looked up
and validated rather than picked from a list, so a typo'd ID is a real,
handled failure case rather than something impossible to enter.
"""
...
def handle_update_incident(incident_service: IncidentService):
    # Spec calls for selecting the incident to update by its ID, so an
    # invalid ID is a real, testable input path (not something a menu
    # pick could ever produce). The active list is shown first purely as
    # a reference so the operator knows what IDs exist.
    ...
    display.print_active_incidents(updatable_incidents)
    print()
    incident_id = cli_utils.prompt_free_text("Enter the incident ID to update: ")
    incident = incident_service.get_incident_by_id(incident_id)

    if incident is None:
        display.print_incident_not_found(incident_id)
        return
