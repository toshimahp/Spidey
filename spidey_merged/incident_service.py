...
class IncidentService:
    ...
    def find_duplicate(self, incident_type: str, location: str):
        """
        Return an existing active incident sharing the same type and
        location, if any — merged in from the intake teammate's duplicate
        check, so the same fire at the same address doesn't silently get
        logged twice. Resolved incidents don't count as duplicates.
        """
        for incident in self._incidents.values():
            if (
                incident.is_active()
                and incident.incident_type == incident_type
                and incident.location == location
            ):
                return incident
        return None
